...
import './css/ApplicantItem.css';

const ApplicantItem = ({user, profile}) => {
    ...
    return(
        <Card type='bump'>
            <div className='applicant-list-item'>
                ...
                <div className='next'>
                    <Link to={Routes.profileSummary.url.replace(':user_id', _id)} ><i className="fas fa-chevron-right" /></Link>
                </div>
            </div>
        </Card>
    )
